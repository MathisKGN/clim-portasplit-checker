#!/usr/bin/env python3
"""
Génère seeds_france.py en combinant :
  1. Les seeds IDF existants (24 points, déjà testés et fonctionnels)
  2. Des seeds auxiliaires pour l'anneau 50-200km (villes réelles avec magasins LM)
  3. Les seeds set-cover calculés pour combler les gaps

Le but : couvrir les 55 magasins à <= 200km de Paris de façon robuste, en
s'appuyant sur les seeds qui marchent déjà + densification.
"""
...
# Seeds IDF existants (déjà prouvés, couvrent les ~36 magasins IDF)
# On les réimporte directement depuis seeds_idf.py sans les dupliquer.
# On ajoute juste les seeds de l'anneau 50-200km.

# Seeds anneau : villes moyennes à 50-200km de Paris où il y a un magasin LM.
# Placés sur la ville réelle (le magasin est dedans ou à côté), garantit qu'on
# "voit" ce magasin dans le top-11.
RING_SEEDS = [
    # 50-100 km (couronne immédiate, parfois déjà dans SEEDS_IDF)
    ...
    # 100-150 km
    ("Rouen/Le Havre (76)",     49.4400, 1.0900),  # Rouen
    ("Le Havre (76)",           49.4900, 0.1100),  # Le Havre
    ...
    ("Valenciennes (59)",       50.3500, 3.5200),  # La Sentinelle
    ("Maubeuge/Hautmont (59)",  50.2800, 4.0000),
    ("Béthune/Verquin (62)",     50.5300, 2.6400),
    ("Lille (59)",              50.6300, 3.0600),  # Lesquin/Neuville/Villeneuve
    ("Caen (14)",               49.1800, -0.3600),  # ~201km, à la limite
]

# Génère le fichier seeds_france.py qui importe SEEDS_IDF + ajoute RING_SEEDS
out = '''"""
...
for name, lat, lon in RING_SEEDS:
    out += f'    ("{name}", {lat:.4f}, {lon:.4f}),\n'
...
(HERE / "stockmonitor" / "seeds_france.py").write_text(out, encoding="utf-8")
print(f"[+] stockmonitor/seeds_france.py généré : {24 + len(RING_SEEDS)} seeds ({24} IDF + {len(RING_SEEDS)} anneau)")
